...
async function loadLabels(filename: string): Promise<Int32Array[]> {
  const buffer = await fetchOnceAndSaveToDiskWithBuffer(filename);
...
class MnistDataset {
  private dataset: [Float32Array[], Int32Array[], Float32Array[], Int32Array[]];
  private trainSize: number;
  private testSize: number;
  private trainBatchIndex: number;
  private testBatchIndex: number;
  constructor() {
    this.dataset = null;
    this.trainSize = 0;
    this.testSize = 0;
...
export const dataset = new MnistDataset();
